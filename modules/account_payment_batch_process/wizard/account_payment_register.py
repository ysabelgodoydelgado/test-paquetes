...
class AccountPaymentRegister(models.TransientModel):
    ...
    def get_payment_values(self, group_data=None):
        ...
        if group_data:
            ...
            res = {
                ...
                "foreign_payment_currency_rate":group_data["foreign_payment_currency_rate"],
            }
        return res

    def _check_amounts(self):
        _logger.info("self.total_amount %s",self.total_amount)
        _logger.info("self.cheque_amount %s",self.cheque_amount)
    ...
    def update_group_pay_data(
        ...
        if self.communication:
            memo = str(data_get.invoice_id.name) + " - " + self.communication
        else:
        ...
        if not name:
            name = "Counterpart"
        print("ESTA ES DATA GET PARA VERIFICAR MONTOS ***************** ",data_get)
        inv_val = {
            "line_name": name,
            "amount": data_get.amount,
            "payment_difference_handling": data_get.payment_difference_handling,
        ...
        group_data.update(
            {
                partner_id: {
                    ...
                    "foreign_payment_currency_rate":data_get.foreign_payment_currency_rate,
                }
            }
        )
            ...
            group_data[partner_id].update(
                {
                    ...
                    "check_amount_in_words": check_amount_in_words,
                    "foreign_payment_currency_rate":line.foreign_payment_currency_rate,
                }
            )
            # prepare name
            name = ""
    ...
    def make_payments(self):
        ...
        payment_ids = []
        _logger.info("**************************ESTE ES EL GROUP DATAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA %s",group_data)
        for partner in list(group_data):
            ...
            #ojo aqui con las facturas, usar inv_val siempre para no usar active ids ya que viene desde codigo 
            ...
            _logger.info("PAYMENT LINES PARA CONCILIAR %s",payment_lines)

            _logger.info("INVOICES A BUSCAR POR EL ACTIVE IDS %s",context.get("active_ids"))
            invoices = self.env["account.move"].browse(context.get("active_ids"))
            _logger.info("INVOICES ENCONTRADAS EN SEARC AVTIVES %s",invoices)
            lines = invoices.line_ids.filtered_domain(domain)
            _logger.info("LINEAS DEESPUES DEL FILTERED POR DOMAIN %s",lines)
            #group_data[partner]["inv_val"][inv.id]["amount"] 
            for account in payment_lines.account_id:
                o = payment_lines.filtered_domain([("account_id", "=", account.id), ("reconciled", "=", False)])

            lines_already_concile = []
            #probar esto para o todos
            for l in lines:
                _logger.info("l.move_id.id %s",l.move_id.id)
                #if (group_data[partner]["inv_val"][l.move_id.id]["payment_difference_handling"]== "open" and group_data[partner]["inv_val"][l.move_id.id]["payment_difference"] >0):
                #    _logger.info("esto bonando menos que el total que se debe y quiero dejarla abierta")
                _logger.info("ESTE ES LA DATA EN EL PARTIALLLLLLLLLLLLLLLLLLLLLLLLLLLLLL %s",group_data[partner]["inv_val"][l.move_id.id])
                am_to_partial = group_data[partner]["inv_val"][l.move_id.id]["amount"] if group_data[partner]["inv_val"][l.move_id.id]["amount"] <= l.move_id.amount_residual else l.move_id.amount_residual
                partials_vals_list = {
                    'amount': am_to_partial,
                    'debit_amount_currency': am_to_partial,
                    'credit_amount_currency': am_to_partial,
                    'debit_move_id': l.id,
                    'credit_move_id': o.id,
                }
                partials = self.env['account.partial.reconcile'].create(partials_vals_list)
                lines_already_concile.append(l)
                _logger.info("PARTIAL REALIZADO %s",partials)
            #lines = lines - lines_already_concile
    
            """for account in payment_lines.account_id:
                _logger.info("account_id de payment line %s",account.name)
                o = (payment_lines + lines).filtered_domain([("account_id", "=", account.id), ("reconciled", "=", False)])
                _logger.info("LINEAS Y PAGOS FILTRADA, lo que mandare a reconcile %s",o)
                (payment_lines + lines).filtered_domain(
                    [("account_id", "=", account.id), ("reconciled", "=", False)]
                ).reconcile()"""
            #arriba mando a conciliar las que dicen reconciled false es decir seria por primera vez? duda con la cuenta a odonde va la diferencia
            #abajo es si alguna tiene la opcion de mantener abierta
            if any(
                ...
                for inv in invoices:
                    #if (group_data[partner]["inv_val"][inv.id]["payment_difference_handling"]== "open" and group_data[partner]["inv_val"][inv.id]["payment_difference"] ==0):
                    #    _logger.info("aunque dice dejar abierta la esta pagando toda")
                    ...
                    for line in inv.line_ids:
                        if line.amount_residual > 0 and payment_state == "paid":
                            _logger.info("entro en el if de residual > 0 y estado pagado")
                            """line_amount = 0.0
                        ...
                        if line.reconciled and payment_state == "partial":
                            _logger.info("entro aqui en linea Reconciled y pago parcial")
                    ...
                    _logger.info("llamar _compute_payments_widget_reconciled_info")
                    inv._compute_amount()
                    inv._compute_payments_widget_reconciled_info()
        if context.get("maxcam_batch", False):
            return payment_ids
